...
1.  **Automated web scraping** of the legacy SI-PNI Web system
    (recommended for data up to 2022).
2.  **Manual data ingestion**, where the user downloads the data
    directly from the official DATASUS dashboard and the function
    performs only cleaning, harmonization, and standardization (required
    for 2023 onwards).
...
4.  **product**: Specifies the specific vaccine or immunobiological
    ...
    - The valid options for product depend strictly on the chosen
      **strategy**.
    - Note: If set to `NULL`, a selection menu will appear.
5.  **data**: Optional path to a local `.xlsx` file downloaded manually
    from the DATASUS dashboard.
...
In this case, the user must **manually download the data**, and
`load_vaccines()` will be responsible only for:
...
6.  Provide the downloaded file to `load_vaccines()` using the `file`
    argument.
