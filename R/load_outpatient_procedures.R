...
load_outpatient_procedures <- function(dataset,
  ...
  filenames_all <- RCurl::getURL(url, ftp.use.epsv = TRUE, dirlistonly = TRUE) %>% stringr::str_split("\r*\n") %>% unlist()

  ####################
  ## Filter Files by Year and State ##
  ####################
  # Determine positions of year and state in the filename
  # Most SIASUS files: 1-2: dataset, 3-4: state, 5-6: year, rest: sequence
  ...
  filenames <- filenames_all[
    substr(filenames_all, 1,2) == param$suffix &
    substr(filenames_all, 3,4) %in% states_filter &
    substr(filenames_all, 5,6) %in% years_filter
  ]
  ...
  ## Harmonize Names ##
  ####################
  dat_mod <- dat %>% tibble::as_tibble() %>% dplyr::rename_with(~ dplyr::recode(., !!!var_names_lookup))
